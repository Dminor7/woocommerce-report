...
# after={0}&
ENDPOINTS = {
    "orders": "orders?orderby=date&order=asc&per_page=100&page={0}&after={1}&before={2}",
...
def get_endpoint(endpoint, *args):
    ...
    return ENDPOINTS[endpoint].format(*args)
...
def get_orders(page_number, order_after, order_before):
    result = []
    while True:
        endpoint = get_endpoint("orders", page_number, order_after, order_before)
...
def download_report(order_after, order_before):
    orders = get_orders(1, order_after, order_before)
    ...
    orders_df = (
        df["line_items"]
        .apply(pd.Series)
        .merge(df, left_index=True, right_index=True, how="outer")
    )
